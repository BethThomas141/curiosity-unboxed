...
function SuccessPage({ onBack }) {
 ...
 width={width}
      ...
      />
<Button className={"d-flex justify-content-center align-items-center purple-button mt-2"} onClick={onBack}><ChevronLeft/> Back</Button>
      ...
      <Container className={"d-flex justify-content-center"}>
        ...
        <div className={"success-message-poem"}>{successPoem.split('\n').map((line, index) => (
        line === "br" ? <div key={index} style={{height: '20px'}}/> : <p className={"text-center mb-2"} key={index}>{line}</p>
      ))}</div>
          </div>
        </div>
      </Container>
      <Container className={"d-flex px-lg-5 text-white text-center success-message-container"}>
        <p>{successMessage}</p>
