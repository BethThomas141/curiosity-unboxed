...
import {Button, Card, CardBody, CardHeader, Col, Collapse, Container, Row, Stack} from "react-bootstrap";
import HiddenSection from "./HiddenStep";
...
<Col className={"d-flex justify-content-end align-items-center "} >
  <ChevronUp className={`float-end ${showCard ? 'show-icon' : 'hide-icon'} `} />
        ...
        <div className="m-0 p-0">
          <CardBody>
              <Stack gap={3} className={"d-flex"}>
                {steps.map((subHint, index) => <HiddenStep index={index + 1} hint={subHint} />)}
              </Stack>
          </CardBody>
        </div>
      </Collapse>
